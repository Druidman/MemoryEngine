...
app.get(
  ...
  async (ctx) => {
    ...
    if (sessionError) ctx.json({ error: sessionError.message }, 500);

    const messages = session_messages?.map((e) => e.payload) ?? [];

    if (!process.env.CHAT_MODEL) ctx.json({ error: "Model not set" }, 500);
    ...
    if (insertError) ctx.json({ error: insertError }, 500);

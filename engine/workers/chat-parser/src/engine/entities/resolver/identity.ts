...
import {
	...
	MappedExtractedEntityRelationWithMentionsAndExternalIdsAndRefType,
...
export function assignLocalIdsToEntityExtraction(
	...
	const mappedData = extractionResult.map((memoryExtraction) => {
		const mappedEntities: MappedExtractedEntityType[] = [...memoryExtraction.entities, USER_ENTITY].flatMap((entity) => {
			return [
				{
					...entity,
		...
		const mappedRelations: MappedExtractedEntityRelationType[] = memoryExtraction.relations.map((relation) => {
			...
			const foundSubject = mappedEntities.find((entity) => entity.canonical_name == subject.canonical_name && entity.type == subject.type);
			const foundObject = mappedEntities.find((entity) => entity.canonical_name == object.canonical_name && entity.type == object.type);

			if (!foundSubject || !foundObject) {
				const message = `[assignLocalIdsToEntityExtraction]: ${
					!foundSubject && !foundObject ? 'Subject and Object' : !foundSubject ? 'Subject' : 'Object'
...
async function assignExternalRefToRelation(
	...
	const { data, error: error } = await supabaseClient.rpc('get_matching_id_for_relation', {
	...
	if (error) {
		logExtractionPipeline('Error in `assignExternalRefToRelation` when fetching data from database');
		throw error;
	}
...
async function assignExternalRefToEntity(
	...
	const { data, error: error } = await supabaseClient.rpc('get_matching_id_for_entity', {
